...
public class MainActivityTest {
    ...
    public void testSwitch() {
        ...
        onView(withId(R.id.button_add)).check(doesNotExist());
    }

    @Test
    public void testCityConsistency() {
        String city_name = "Edmonton";
        onView(withId(R.id.button_add)).perform(click());
        onView(withId(R.id.editText_name)).perform(ViewActions.typeText(city_name));
        onView(withId(R.id.button_confirm)).perform(click());
        onData(is(instanceOf(String.class))).inAdapterView(withId(R.id.city_list)).atPosition(0).perform(click());
        onView(withId(R.id.text_city)).check(matches(withText(city_name)));
